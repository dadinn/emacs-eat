...
(defun eat--select-new-terminal-name (pattern &rest args)
  "Prompt to read name for new terminal.

Prompt is generated by formatting PATTERN with ARGS."
  (let ((name (eat--select-terminal (apply (function format) pattern args))))
    (cond
     ((string= name eat-buffer-name)
      (eat--select-new-terminal-name "New terminals cannot be named \"%s\"! Try again: " eat-buffer-name))
     ((assoc name eat--terminal-buffers)
      (eat--select-new-terminal-name "Terminal named \"%s\" already exists! Try again: " name))
     (t name))))

(defun eat-rename-terminal ()
  (interactive)
  (let* ((original-buffer-name (buffer-name))
         (named-terminal? (not (string= original-buffer-name eat-buffer-name)))
         (original-terminal-name
          (seq-filter (lambda (pair) (string= (cdr pair) original-buffer-name)) eat--terminal-buffers))
         (original-terminal-name (caar original-terminal-name))
         (original-terminal-name
          (if named-terminal? (format "terminal \"%s\"" original-terminal-name) "default terminal"))
         (target-terminal-name (eat--select-new-terminal-name "Rename %s to: " original-terminal-name))
         (target-buffer-name (format-terminal-buffer-name target-terminal-name)))
    (with-current-buffer original-buffer-name
      (rename-buffer target-buffer-name))
    (cond
     ((not (string-empty-p target-terminal-name))
      (setq eat--terminal-buffers
       (cons (cons target-terminal-name target-buffer-name)
             (seq-filter (lambda (pair) (not (string= original-terminal-name (car pair))))
                         eat--terminal-buffers)))
      (message "Renamed terminal %s to \"%s\"" original-terminal-name target-terminal-name))
     (t
      (setq eat--terminal-buffers
       (seq-filter (lambda (pair) (not (string= original-terminal-name (car pair))))
                   eat--terminal-buffers))
      (message "Renamed terminal %s as the default terminal" original-terminal-name)))))
