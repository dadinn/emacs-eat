...
(defcustom eat-line-default-mode nil
  "Use `eat-line-mode' instead of `eat-semi-char-mode' as default minor mode
when opening a new terminal or leaving char mode."
  :type 'boolean
  :group 'eat-ui)

(defun eat-default-mode ()
  (interactive)
  (if eat-line-default-mode
      (eat-line-mode)
    (eat-semi-char-mode)))
...
(defun eat--select-terminal-name (cannot-exist pattern &rest args)
  "Prompts to enter a terminal name, and provides completion to select from
already registered ones.

The CANNOT-EXIST option checks whether it is allowed to select existing
already registered names. A terminal name cannot be empty, and must not
contain characters < or >. Prompt is generated by formatting of PATTERN
using ARGS."
  ...
  (let* ((program (or program (funcall eat-default-shell-function)))
    ...
    (with-current-buffer terminal-buffer
      ...
      (unless (and eat-terminal
        ...
        (eat-exec terminal-buffer "eat" "/usr/bin/env" nil
                  (list "sh" "-c" program)))
      terminal-buffer)))
