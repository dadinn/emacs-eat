...
(defvar eat-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [?\C-c ?\C-\S-k] #'eat-kill-process)
    ...
    (define-key map [?\C-c ?\C-m] #'eat-rename-terminal)
    (define-key map [?\C-c ?\C-z] #'switch-to-prev-buffer)
    (define-key map [?\C-c ?\C-u] #'eat-reset)
...
(defvar eat--named-terminals ())

(defun eat--validate-terminal-name (terminal-name continue cannot-exist)
  (cond
   ((string-match "[<>]" terminal-name)
    (funcall continue cannot-exist "Terminal name cannot contain < or > characters! Try again: "))
   ((string-empty-p terminal-name)
    (funcall continue cannot-exist "Terminal name cannot be empty! Try again: "))
   ((and cannot-exist (member terminal-name eat--named-terminals))
    (funcall continue cannot-exist "Terminal named \"%s\" already exists! Try again: " terminal-name))
   (t terminal-name)))

(defun eat--select-terminal-name (cannot-exist pattern &rest args)
    "Prompts to enter a terminal name, and provides completion to select from already registered ones.

The CANNOT-EXIST option checks whether it is allowed to select existing already registered names.
A ternminal name cannot be empty, and must not contain characters < or >.
Prompt is generated by formatting PATTERN with ARGS."
  (let* ((prompt (apply (function format) pattern args))
         (terminal-name (completing-read prompt (and (not cannot-exist) eat--named-terminals))))
    (eat--validate-terminal-name terminal-name (function eat--select-terminal-name) cannot-exist)))

(defun eat--format-buffer-name (terminal-name)
  (cond
   ((or (not terminal-name) (string-empty-p terminal-name))
    eat-buffer-name)
...
DISPLAY-BUFFER-FN is the function to display the buffer."
  (let* ((program (or program (funcall eat-default-shell-function)))
         (terminal-name (and arg (eat--select-terminal-name nil "Terminal name: ")))
         (terminal-buffer
          (get-buffer-create
           (if terminal-name
               (eat--format-buffer-name terminal-name)
             eat-buffer-name))))
    (when terminal-name
      (add-to-list
    ...
    (with-current-buffer terminal-buffer
      (unless (eq major-mode #'eat-mode)
        (eat-mode))
      (funcall display-buffer-fn terminal-buffer)
      (unless (and eat-terminal (eat-term-parameter eat-terminal 'eat--process))
        (eat-exec terminal-buffer "eat" "/usr/bin/env" nil (list "sh" "-c" program)))
      terminal-buffer)))
         ...
         (target-terminal-name
          (and (not arg)
           (eat--select-terminal-name t "Rename %s to: "
            (if (format "terminal \"%s" original-terminal-name)
                "default terminal"))))
         (target-buffer-name
...
Start a new un-named Eat session, or switch to an already existing one.
Return the buffer selected (or created).

With a prefix ARG, create a new named session, or switch to an already
existing one by selecting a session name.

With double prefix argument ARG, ask for the program to run,
and run it in a newly created or existing named session by selecting
a session name.
